...
export function CanonicalActionsPanel(props: {
  ...
  const evidence = useMemo(() => props.canonical?.visionEvidence || [], [props.canonical?.visionEvidence]);
  const unitTypes = useMemo(() => props.canonical?.unitTypes || [], [props.canonical?.unitTypes]);
  ...
  async function act(path: string, method: 'POST' | 'PUT', body?: unknown, success = 'Canonical record updated.') {
    ...
    try {
      await expectOk(await fetch(`/api/vision/jobs/${props.jobId}${path}`, {
