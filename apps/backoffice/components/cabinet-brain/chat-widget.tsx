...
type RealtimeSession = {
  client_secret?: { value?: string; token?: string } | string;
...
export function ChatWidget() {
  ...
  async function optimizeInput() {
    ...
    try {
      const response = await fetch('/api/vision/assistant/optimize', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ jobId, prompt }) });
  ...
  function extractEphemeralKey(session: RealtimeSession): string | null {
    if (typeof session.client_secret === 'string') return session.client_secret;
    return session.client_secret?.value || session.client_secret?.token || session.ephemeral_key || null;
  }
  ...
  async function runAgenticTurn(userMsg: string, withVoiceOutput: boolean) {
    ...
    try {
      ...
      setVoiceStatus(`Agent turn failed: ${error instanceof Error ? error.message : 'unknown error'}`);
    } finally {
      setIsLoading(false);
    }
  }
  ...
  async function initVoiceSession() {
    try {
      stopListening();
      stopRealtimeConnection();
      const response = await fetch('/api/vision/chat/voice/session', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      setVoiceStatus(`Fallback voice mode available: ${reason}`);
    } catch (error: unknown) {
      setVoiceTransport('fallback');
      setVoiceStatus(`Realtime unavailable. Browser voice fallback available: ${error instanceof Error ? error.message : 'unknown error'}`);
    }
  }
