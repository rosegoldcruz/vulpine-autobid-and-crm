...
import { cabinetBrainDownloadUrl } from './vision-links';
...
// The gateway has many endpoint-specific payloads; callers narrow the successful data shape.
...
function readPointer(): { projectId?: string; jobId?: string } {
  ...
  try {
    ...
    const queryJobId = new URLSearchParams(window.location.search).get('job') || undefined;
    const pointer = typeof parsed === 'object' && parsed ? parsed : {};
    return {
      projectId: typeof pointer.projectId === 'string' ? pointer.projectId : undefined,
      jobId: queryJobId || (typeof pointer.jobId === 'string' ? pointer.jobId : undefined),
    };
  } catch { return {}; }
}
...
export function WorkspaceClient() {
  ...
  const loadSnapshot = useCallback(async (jobId: string) => {
    const data = await parseApiResponse(await fetch(`/api/vision/jobs/${jobId}/workspace`, { cache: 'no-store' }));
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const list = await parseApiResponse(await fetch('/api/vision/projects', { cache: 'no-store' }));
        if (!alive) return;
  ...
  async function createProject() {
    ...
    try {
      const data = await parseApiResponse(await fetch('/api/vision/projects', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ projectName }) }));
  ...
  async function uploadFiles() {
    ...
    try {
      ...
      const data = await parseApiResponse(await fetch('/api/vision/uploads', { method: 'POST', headers: { 'x-project-id': project.projectId }, body: form }));
  ...
  async function processJob() {
    ...
    try {
      const runData = await parseApiResponse(await fetch(`/api/vision/jobs/${job.id}/runs`, { method: 'POST' }));
      setRun(runData.run);
      eventSource = new EventSource(`/api/vision/runs/${runData.run.id}/events`);
      ...
      await parseApiResponse(await fetch(`/api/vision/runs/${runData.run.id}/progress`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ stage: 'validating', unit: 'files', completed: job.manifest.files.length, total: job.manifest.files.length, message: 'Server accepted the manifest.' }) }));
      await parseApiResponse(await fetch(`/api/vision/jobs/${job.id}/process`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ jobId: job.id }) }));
  ...
  async function controlRun(action: 'pause' | 'resume' | 'cancel' | 'retry' | 'remove') {
    ...
    try {
      if (action === 'remove') {
        await parseApiResponse(await fetch(`/api/vision/runs/${run.id}/control`, { method: 'DELETE' }));
        setRun(null); setDiagnostics(null); setProgress([]); setNotice('Inactive processing run removed from the queue.');
      } else {
        const data = await parseApiResponse(await fetch(`/api/vision/runs/${run.id}/control`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action, reason: 'Operator action from workspace' }) }));
  ...
  async function exportBid(format: 'json' | 'csv' | 'xlsx' | 'review_pdf', audience: 'internal_review' | 'customer') {
    ...
    try {
      await parseApiResponse(await fetch(`/api/vision/jobs/${job.id}/export/approve`, {
        method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ note: `Explicit ${audience} export approval from the Cabinet Brain workspace.` }),
      }));
      const data = await parseApiResponse(await fetch(`/api/vision/jobs/${job.id}/export`, {
        method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ format, audience }),
      }));
      window.open(cabinetBrainDownloadUrl(data.downloadUrl), '_blank', 'noopener,noreferrer');
  ...
  return (
    <div className="workspace-shell">
      ...
      <main className="workspace-main">
        ...
        {activeView === 'Pipeline' ? <BackofficeDashboard/> : activeView === 'Settings' ? <ProcessingSettingsPanel/> : <><div className="workspace-grid"><div className="workspace-content"><div className="workspace-upper"><IngestionPanel job={job} queue={fileQueue} manifest={ingestionManifest} files={files} setFiles={setFiles} onUpload={uploadFiles} onQueueChanged={async () => { if (job) await loadSnapshot(job.id); }} canUpload={canUpload} busy={busy}/><WorkflowPanel job={job} canonical={canonical} run={run} diagnostics={diagnostics} progress={progress} onProcess={processJob} onControl={controlRun} busy={busy}/></div><CanonicalActionsPanel jobId={job?.id} canonical={canonical} onChanged={async () => { if (job) await loadSnapshot(job.id); }}/><ReviewTable job={job} canonical={canonical} onEvidence={(id) => { setFocusEvidenceId(id); document.getElementById('blueprint-title')?.scrollIntoView({ behavior: 'smooth' }); }}/></div><QaPanel job={job} canonical={canonical} integrations={integrations} projectId={project?.projectId} projectName={project?.projectName} onExport={exportBid} busy={busy}/></div><BlueprintStudio projectId={project?.projectId} sheets={planSheets} canonical={canonical} focusEvidenceId={focusEvidenceId} onChanged={async () => { if (job) await loadSnapshot(job.id); }}/></>}
      </main>
    </div>
  );
}
