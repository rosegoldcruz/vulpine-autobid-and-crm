...
/* Blueprint measurements require raw image dimensions and must bypass Next image transforms. */
...
export function BlueprintStudio(props: {
  ...
  async function saveMeasurement() {
    ...
    try {
      await expectOk(await fetch(`/api/vision/projects/${props.projectId}/measurements`, {
  ...
  async function saveMarkup() {
    ...
    setBusy(true); setMessage(undefined);
    try {
      await expectOk(await fetch(`/api/vision/projects/${props.projectId}/evidence-snippets`, {
  ...
  async function reviewClassification(classification: string) {
    ...
    setBusy(true); setMessage(undefined);
    try {
      await expectOk(await fetch(`/api/vision/plan-sheets/${active.id}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ classification, note: classificationNote }) }));
  ...
  async function saveEvidence(kind: 'vision' | 'snippet') {
    ...
    try {
      ...
      const url = kind === 'vision' ? `/api/vision/projects/${props.projectId}/vision-evidence` : `/api/vision/projects/${props.projectId}/evidence-snippets`;
  ...
  return (
    <section className={`work-panel blueprint-studio${fullscreen ? ' fullscreen' : ''}`} aria-labelledby="blueprint-title">
      ...
      {!active ? <div className="empty-table">Rendered pages appear here after processing.</div> : <div className="blueprint-layout">
        ...
        <div className="blueprint-canvas" aria-label="Scrollable, pannable blueprint canvas"><div className={`blueprint-transform${actualSize ? ' actual-size' : ''}`} style={{ transform: `scale(${zoom}) rotate(${rotation}deg)` }}><img src={`/api/vision/plan-sheets/${active.id}/image`} alt={`${active.sourceFileName}, page ${active.pageNumber}`} onClick={choosePoint}/>{activeEvidence.map((item) => <button type="button" title={`${item.kind} evidence ${item.id}`} aria-label={`Evidence overlay ${item.kind}`} className={item.id === props.focusEvidenceId ? 'evidence-overlay focused' : 'evidence-overlay'} key={item.id} style={{ left: `${item.region!.x * 100}%`, top: `${item.region!.y * 100}%`, width: `${item.region!.width * 100}%`, height: `${item.region!.height * 100}%` }} onClick={(event) => { event.stopPropagation(); setMessage(`${item.kind.replace('_', ' ')} evidence ${item.id}`); }}/>) }{points.map((point, index) => <span className="measure-point" key={`${point.x}-${point.y}-${index}`} style={{ left: `${point.x * 100}%`, top: `${point.y * 100}%` }}>{index + 1}</span>)}</div></div>
