...
type DroppedFileEntry = {
  ...
  createReader: () => {
    readEntries: (resolve: (entries: DroppedFileEntry[]) => void, reject: (reason?: unknown) => void) => void;
  };
};
...
async function filesFromEntry(entry: DroppedFileEntry): Promise<File[]> {
  if (entry?.isFile) {
...
export function IngestionPanel(props: {
  ...
  async function cancel(item: WorkspaceFileQueueItem) {
    if (!props.job) return;
    const response = await fetch(`/api/vision/jobs/${props.job.id}/file-queue?itemId=${encodeURIComponent(item.id)}`, { method: 'DELETE' });
  ...
  return (
    <section className="work-panel ingestion-panel" aria-labelledby="ingestion-title">
      ...
      <div className="upload-bar">
        ...
        <label className="file-picker folder-picker"><FolderOpen size={15}/><span>Choose folder</span><input type="file" multiple {...directoryInputProps} onChange={(event) => choose(event.target.files, true)} /></label>
