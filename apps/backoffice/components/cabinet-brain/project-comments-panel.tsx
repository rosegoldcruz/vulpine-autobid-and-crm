...
export function ProjectCommentsPanel({ projectId }: { projectId: string }) {
  ...
  const load = useCallback(async () => {
    const response = await fetch(`/api/vision/projects/${projectId}/comments`, { cache: 'no-store' });
  ...
  async function add() {
    const response = await fetch(`/api/vision/projects/${projectId}/comments`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ body, context: 'cabinet_review' }) });
    const payload = await response.json();
