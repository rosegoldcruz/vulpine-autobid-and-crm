...
export function BidOutreachPanel(props: { projectId: string; jobId: string; projectName: string }) {
  ...
  const refresh = useCallback(async () => {
    const [data, exports] = await Promise.all([
      responseData(await fetch(`/api/vision/backoffice/projects/${props.projectId}/outreach`, { cache: 'no-store' })),
      responseData(await fetch(`/api/vision/jobs/${props.jobId}/export`, { cache: 'no-store' })),
    ]);
  ...
  async function prepare() {
    ...
    try {
      const data = await responseData(await fetch(`/api/vision/backoffice/projects/${props.projectId}/outreach`, {
  ...
  return <section className="inspector-card outreach-card">
    ...
    {draft && !connected ? <p className="microcopy">Draft remains server-persisted. Sending stays disabled until a verified email provider is configured.</p> : null}
